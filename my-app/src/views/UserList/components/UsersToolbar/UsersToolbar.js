...
const UsersToolbar = (props) => {
  ...
  const handleDeleteUserSubmit = (event) => {
    event.preventDefault()
    deleteUser(deleteUserValues.id).then((res) => {
      handleCloseDelete()
      onUpdate()
    }).catch((err) => {
      console.log(err)
  ...
  const deleteBody = (
    <div
      ...
      className={classes.paper}
    >
      <h2 id='simple-modal-title'>Delete {deleteUserValues.name} with ID {deleteUserValues.id}</h2>
      <Button onClick={(event) => handleDeleteUserSubmit(event)} type='submit'>Submit</Button>
    </div>
  )
